...
<p align="center">
  Настольный мост совместимости между подписками Happ и другими VPN-клиентами
</p>
...
> [!IMPORTANT]
> **0.1.6** — первый стабильный публичный релиз. В него входят сборки Linux и Windows, безопасная диагностика, копирование готовых профилей, сведения о трафике и сроке подписки, а также надёжное преобразование подписок. Версия для Windows проверена вручную и работает исправно.

Happwner PC — в первую очередь мост совместимости для подписок, рассчитанных на Happ. Некоторые провайдеры выдают ссылки, шифрование или привязанные к устройству ответы, которые обычные VPN-клиенты не могут импортировать напрямую. Happwner PC обрабатывает эти форматы и отдаёт готовые стандартные профили по постоянной локальной ссылке. Это позволяет использовать подписку, к которой у вас есть законный доступ, в Happ и других совместимых клиентах.

Локальную ссылку можно использовать в VPN-клиенте на том же компьютере или раздать телефонам, телевизорам и другим устройствам в доверенной домашней сети. Мост также принимает несколько исходных форматов, не связанных с Happ, поэтому проект не ограничен только Happ-подписками.
...
- **Проверка подписок.** Приложение показывает HTTP-статус, размер ответа, найденные профили, протоколы, предупреждения конверсии и понятную ошибку обработки.
...
- провести smoke-тест трея и автозапуска в GNOME и других Linux DE;
